...
class OTW_Post_Template_Grid_Manager extends OTW_Component{
    ...
    $content .= "\n<p class=\"otw_grid_manager_info_block\" id=\"".$this->meta_name."_info_block\">";
		...
		$content .= "\njQuery(document).ready(function(){
				otw_grid_manager_".$this->meta_name." = new otw_post_template_grid_manager_object( '".$this->meta_name."', ".json_encode( $this->labels ).", ".json_encode( $js_templates )."  );";
		$content .= "\n});";
		$content .=  "\n</script>";
		$content .= "</div>";
		echo $content;
							...
							if( count( $column->shortcodes ) ){
								...
								foreach( $column->shortcodes as $shortcode ){
									...
									if( preg_match( "/^\[otw_shortcode_post_[^\s^\]*]+/", $grid_shortcode_code, $shortcode_matches ) ){
										$grid_shortcode_code = preg_replace( "/^(\[otw_shortcode_post_[^\s^\]*]+)/", '$1 post_item_id='.$this->post_item_id, $grid_shortcode_code );
									}
